...
# Load game data
resources = load_json_file('data/resources.json')
...
# Callback query handler for checking inventory
...
async def check_inventory(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.reply(player_status, parse_mode=ParseMode.MARKDOWN)

# Register function for main.py
def register(app: Client):
    app.on_message(filters.command("explore"))(explore)
    app.on_callback_query(filters.create(maintenance_mode_only) & filters.create(user_verification))(handle_callback_query)
